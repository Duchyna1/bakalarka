...
$$\forall X_1, \dots, X_k ((P_1 \land \cdots \land P_i \land \lnot P_{i+1} \land \cdots \land \lnot P_n) \Rightarrow Q)\text{,}$$
...
\begin{definition}
    Klauzulu typu $p(t_1, \dots, t_n) \leftarrow \tp.$, kde $p$ je $n$-árny predikát a $t_1, \dots, t_n$ sú termy, nazývame \textbf{fakt}. Klasicky ich skrátene zapisujeme ako $p(t_1, \dots, t_n).$ (zdôrazňujem, že bodka na konci je súčasťou skráteného zápisu). Keďže ale často potrebujeme vyjadriť pre jeden predikát viacero faktov, zvolíme nasledujúci zápis. Fakty 
    $$p(t_{1, 1}, \dots, t_{1, n}).$$
    $$\vdots$$
    $$p(t_{k, 1}, \dots, t_{k, n}).$$
    zapíšem ako
    $$p = \{(t_{1, 1}, \dots, t_{1, n}), \dots, (t_{k, 1}, \dots, t_{k, n})\}.$$
