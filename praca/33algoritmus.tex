...
Príkladom, aká transformácia sa deje, je transformácia na objekt typu \texttt{Atom} a \texttt{Predicate}.
...
Celá štruktúra a hierarchia tried, ktoré reprezentujú AST, sú podrobnejšie popísané v diagrame.
...
Táto vlastnosť by sa dala vyjadriť pomocou bezkontextovej gramatiky, ale pre jednoduchosť
implementácie sme sa rozhodli proti tomu.
...
$a$, ale naša aktuálna implementácia gramatiky vyžaduje tento zápis). Nech $M$ je množina z definície
\JOIN. Potom $M = \{()\}$, keďže iba pre $()$ platí $() \in \$true$. Zápis je trochu neintuitívny,
ale zhoduje sa s definíciou, keďže neexistuje žiadny term $T_{i, j}$ ani premenná $P_k$. Potom výsledná
relácia $R$ je rovná $\{(n1(), a())\}$.
...
Pôvodne sme chceli mať 5 operátorov. Piaty operátor by bol \DEF, ktorý by nám umožňoval definovať
reláciu pomocou množiny n-tíc. Preto sa používa v programe kompilátora trieda \texttt{RADef}. Keďže
sme sa ale nakoniec rozhodli uprednostniť menšiu množinu operátorov, \DEF\ sa transfomuje na \UNION\
...
\begin{note}
    ...
    môže záležať. Zoberme si napríklad predikát \verb|$plus(X, Y, Z)|, ktorý je pravdivý vtedy a len
    vtedy, keď $X + Y = Z$. Jeho implementácia v databázovom systéme môže byť taká, že ak sú zadané
    (sú konštanty) $X$ a $Y$, vráti $Z$ ($Z$ je premenná) alebo ak sú zadané $X$, $Y$ a $Z$, vráti
    pravdivostnú hodnotu a to je všetko. Aktuálna implementácia napr. \JOIN-u v našom databázovom
    systéme je ale nasledovná. Zoberie sa prvá relácia a jej vstupný vektor. Vyberie sa z nej nejaká
    n-tica, aby spĺňala vstupný vektor. Týtom sa môžu konkretizovať niektoré premenné. Potom sa
    zoberie druhá relácia a jej vstupný vektor (s už konkretizovanými premennými) a spravý sa to
    isté. Preto v našom prípade záleží od poradia atómov. Ako príklad si zoberme program
\begin{verbatim}
    p(Z) :- $cisloDo10(X), $cisloDo10(Y), $plus(X, Y, Z).
\end{verbatim}
    Prislušný výraz v relačnej algebre je
\begin{verbatim}
JOIN(
...
\end{verbatim}
    a tento výraz nemusí byť správne vyhodnotený, keďže $plus$ nemusí vedieť pracovať so stroma
    premennými.
    
    Preto si používateľ musí dávať pozor na to, v akom poradí píše atómy do klauzúl, keďže to môže
    ovplyvniť výsledok. Náš kompilátor zoberie naprv kladné atómy v poradí a následne negované atómy
    v poradí v akom boli zapísané v programe.
\end{note}
...
musíme prípadne predikáty \textit{zabaliť} do rekurzívneho operátora \REC. To spravíme tak, že naprv
...
Výsledný výraz v relačnej algebre začneme stavať od dotazu. Prechádzame jeho výraz do hĺby a
pamätáve si, cez aké \REC\ operátory sme prešli. Keď narazíme na \texttt{RelationRef} (pre ktorý sme
...
relácia, ale iba tie záznemi, ktoré spĺňajú hlavu dotazu. Preto ešte celý výsledný výraz zabalíme do
\JOIN-u, ktorého výstupný vektor bude zodpovedať hlave dotazu.

\medskip
\refstepcounter{priklad}
    Ukážeme si preklad programu spolu z vysvetlením, ktoré časti relačnej algebry zodpovedajú ktorým
    častiam programu.
\begin{verbatim}
 1  node :- {(a()), (b()), (c())}.
 2  blocked :- {(c())}.
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item Postavenie výsledného výrazu v relačnej algebre
    \begin{enumerate}
        \item Postavenie grafu závislosti predikátov
