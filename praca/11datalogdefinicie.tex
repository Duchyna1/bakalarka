...
\begin{definition}
    \textbf{$n$-árna relácia} $r$ je definovaná ako $r \subseteq D_1 \times D_2 \times \dots \times D_n$. Jednotlivé prvky relácie $r$ nazývame \textbf{tuples}, \textbf{záznamy} alebo \textbf{usporiadané $n$-tice}. Jednotlivým zložkám $n$-tíc hovoríme \textbf{atribúty} relácie. Množinám $D_1, D_2, \dots, D_n$ sa hovorí \textbf{domény} (alebo \textbf{typy}) atribútov relácie $r$. 
\end{definition}

\begin{definition}
    TODO prepísať to pre interpretáciu
...
\begin{definition}
    $\forall X \iota $ sa nazýva \textbf{kvantifikovaná implikácia}, kde $X$ je premenná a $\iota$ je implikácia. Ak je kvantifikovaná každá premenná v kvantifikovanej implikácii, nazývame ju \textbf{klauzula}.
\end{definition}
...
\begin{definition}
    ...
    \textbf{Ohodnotenie premenných} je funkcia, ktorá priradí každej premennej konštantu. Ohodnotenie $o$, ktoré premennej $X_i$ priradí konštantu $k_i$ (pre $i \leq n$) označujeme $o(X_1 | k_1, \dots , X_n | k_n)$. Keď vo formule $A$ resp. implikácii $\iota$ resp. teórii $K_1\land \dots \land K_n$ nahradíme všetky premenné podľa ohodnotenia $o$, výslednú formulu označujeme ako $A[o]$ resp. $\iota[o]$ resp. $K_1\land \dots \land K_n[o]$.
\end{definition}

\begin{definition}
    \textbf{Interpretácia jazyka} je funkcia, ktorá každej atomickej formule priradí jednu z hodnôt $\true, \false, \unknown$.
\end{definition}
...
\begin{definition}
    Majme teóriu $K_1 \land \dots \land K_n$ a atomickú formulu $Q$. Potom teóriu $K_1 \land \dots \land K_n \land Q$ nazývame \textbf{teória s dotazom $Q$}.
\end{definition}
...
\begin{definition}
    ...
    $$\{(c_1, c_2, \dots, c_k) \, | \, \inter \models_\true Q[X_1|c_1, X_2|c_2, \dots , X_k|c_k]\}\text{.}$$
    
    Ak dotaz $Q$ neobsahuje žiadnu premennú a platí $\inter \models_\true Q$, výsledkom dotazu je $\true$, inak je výsledkom $\false$.
\end{definition}

\begin{note}
    Môžeme si všimnúť, že dotazom sa vieme pýtať ina otázky typu \textit{platí $p(K_1, \dots, K_n)$?}, kde $K_1, \dots, K_n$ sú konštanty. Nevieme sa pýtať na opak (otázky typu \textit{neplatí... ?}).
    ...
    \land\\
    \left(\lnot q \Rightarrow p\right)\text{.}$$
    Jediná validná interpretácia $\inter_V$ je $\inter_V \models_\unknown p$ a $\inter_V \models_\unknown q$ (ak by sme zobrali ľubovoľnú inú interpretáciu $\inter$, došli by sme k sporu $\inter \models_\true p \, \land \, \inter \not\models_\true p$ resp. $\inter \models_\false p \, \land \, \inter \not\models_\false p$). Ak by sme sa vedeli pýtať na záporné dotazy, dostali by sme pre teórie s dotazom $\mathcal{T} \land p$ a $\mathcal{T} \land \lnot p$ výsledok dotazu $\false$. S našim prístupom dostaneme pre dotaz \uv{\textit{platí $p$?}} $\false$.
\end{note}
