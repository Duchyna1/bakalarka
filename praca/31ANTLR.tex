...
\textbf{L}anguage \textbf{R}ecognition. V tejto časti si priblížime, ako \textit{ANTLR} funguje.
...
\begin{definition}
    ...
    ktorom máme viacero neterminálov, ktoré sa dajú prepisovať, vždy prepíše \textit{najľavejší},
    resp. \textit{najpravejší} neterminál.
\end{definition}

\subsection{Syntaktická analýza}
Na to, aby sme vedeli preložiť datalog do relačnej algebry, zišlo by sa nám vedieť z čoho sa
datalogový program skladá. Aké obsahuje fakty, klauzuly, dotaz, atď. Pomocou týchto informácií
bude následne jednoduchšie napísať algoritmus, ktorý nám datalogový program preloží do relačnej
algebry. Jedným zo spôsobov je tzv. \textit{syntaktická analýza}. Ukážeme si, ako funguje na
jednoduchom príklade, v ktorom budeme prekladať aritmetické výrazy z infixovej notácie do
postfixovej notácie.
...
reťazca. Tieto tokeny sú popísané pomocou regulárnych jazykov (väčšinou pomocou regulárnych
výrazov, ktoré sú ekvivalentné s regulárnymi gramatikami \cite{rovan2013regex}). Následne sa odvodenie robí nad
tokenami, čím sa zmenší veľkosť stromu odvodenia a zjednoduší sa nám odvodenie. V našom prípade
...
\begin{todo}
    Lepšie popísať ako funguje lexer.
\end{todo}
...
touto gramatikou a v prípade, že áno, vygeneruje k nemu prislúchajúci strom ľavého krajného
odvodenia.